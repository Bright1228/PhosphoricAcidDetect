...
from src.signalp6.models import ProteinBertTokenizer, BertSequenceTaggingCRF
...
#     Adamax,
# )
from src.signalp6.training_utils import (
    LargeCRFPartitionDataset,
    SIGNALP_KINGDOM_DICT,
    RegionCRFDataset,
    compute_cosine_region_regularization,
...
from src.signalp6.utils import class_aware_cosine_similarities, get_region_lengths
...
def main_training_loop(args: argparse.ArgumentParser):
    ...
    num_epochs_no_improvement = 0
    early_stopping_patience = 10  # 可以根据需要调整
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        # 检查验证损失和 MCC 是否有改进
        if val_loss < best_val_loss or mcc_sum > best_mcc_sum:
        ...
        if num_epochs_no_improvement > early_stopping_patience:
            logger.info(f"Stopping training as there have been {num_epochs_no_improvement} epochs without improvement.")
            break
    ...
    #         num_epochs_no_improvement += 1
    #
    #         # Check if the number of epochs without improvement exceeds 100
    #     if num_epochs_no_improvement > 100:
    #         logger.info(f"Stopping training as there have been {num_epochs_no_improvement} epochs without improvement.")
    #         break
    #
    #     # Ensure a model has been saved
    #     if not best_model_saved:
    #         logger.warning("Training stopped without saving any model. Please check the training process.")
    #
    #     # when cross-validating, check that the seed is working for region detection
