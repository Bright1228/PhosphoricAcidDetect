...
class CRF(nn.Module):
    ...
    def forward(
        ...
        计算给定发射分数（emission scores）和标签序列（tags）的条件对数似然（conditional log likelihood）
        Args:
            emissions (`~torch.Tensor`): Emission score tensor of size
                ``(seq_length, batch_size, num_tags)`` if ``batch_first`` is ``False``,
                ``(batch_size, seq_length, num_tags)`` otherwise.
                发射分数张量，形状为 (seq_length, batch_size, num_tags)
                如果 batch_first 为 False，否则为 (batch_size, seq_length, num_tags)。
                这个张量表示每个时间步、每个批次样本和每个标签的发射分数

            tags (`~torch.LongTensor`): Sequence of tags tensor of size
                ``(seq_length, batch_size)`` if ``batch_first`` is ``False``,
                ``(batch_size, seq_length)`` otherwise. If none, then tag_bitmap should be provided.
                标签序列张量，形状为 (seq_length, batch_size)
                如果 batch_first 为 False，否则为 (batch_size, seq_length)。
                如果没有提供 tags，则应提供 tag_bitmap
            ...
            tag_bitmap (`~torch.LongTensor`): (`~torch.LongTensor`): Sequence of tags tensor of size
                ...
                标签序列张量，形状为 (seq_length, batch_size, num_tags)
                如果 batch_first 为 False，否则为 (batch_size, seq_length, num_tags)。
                如果没有提供 tags，则应提供 tag_bitmap。

            mask (`~torch.ByteTensor`): Mask tensor of size ``(seq_length, batch_size)``
                if ``batch_first`` is ``False``, ``(batch_size, seq_length)`` otherwise.
                掩码张量，形状为 (seq_length, batch_size) 如果 batch_first 为 False，否则为 (batch_size, seq_length)。
